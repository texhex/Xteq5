...
namespace Yamua
{
    ...
    {
        IProgress<ReportedObject> _innerIProgress;
        bool _burned = false;

        /// <summary>
        /// Provides access to the instance that is beeing reported as "Progress"
        /// </summary>
        public ReportedObject Content = new ReportedObject();
        ...
        public void Report()
        {
            //MTH: We need to make sure that a single instance of this class is used only once because ReportedObject might be
            //used in a entire different thread. Quote from [Reporting Progress from Async Tasks](http://blog.stephencleary.com/2012/02/reporting-progress-from-async-tasks.html) by Stephen Cleary
            ...
            if (_burned == false)
            {
                ...
                if (_innerIProgress != null)
                {
                    _innerIProgress.Report(Content);
                }
            }
            else
            {
